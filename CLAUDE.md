...
- `/mobile-keys/contacts` — Key Code Contacts page. A small inline-edit reference table (Store / Name / Email) of who to reach out to at each store for a key code. Spreadsheet-style: type directly in cells, rows auto-save (debounced 600ms), "+ Add Contact" appends a blank row, the × deletes. Backed by the `key_code_contacts` table.
...
- `key_code_contacts` — backs the Key Code Contacts page under Mobile Keys. One row per contact, columns `store` / `name` / `email` — all free text and nullable. Rows are typed in inline (spreadsheet-style) and auto-saved via the `/mobile-keys/contacts/api/*` JSON endpoints. No dropdowns, no validation; purely a reference list.
