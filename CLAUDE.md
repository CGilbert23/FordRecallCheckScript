...
- `/leads` and `/leads/new`, `/leads/<id>/edit`, `/leads/<id>/delete`, `/leads/<id>/convert` (lead → account). All leads created through the UI are warm prospects (the in-app cold-call workflow was removed; existing cold rows in the DB are no longer surfaced).
- `/cold-leads` — Xtime Follow Up Calls. Native spreadsheet-style page backed by the `cold_leads` Supabase table. Five market tabs (Doylestown / Newtown/Langhorne / Mechanicsburg / Washington / West Chester/Exton — Newtown+Langhorne share a rep, and West Chester+Exton are grouped); each tab renders existing DB rows + ghost rows padded out to 100 so it feels like Excel. Ghost rows auto-promote to DB rows on first input (debounced 500ms POST to `/cold-leads/api/create`); saved rows update via `/cold-leads/api/update/<id>`. Phone numbers auto-format to `(NNN-NNN-NNNN)` on blur. Name/phone blur fires a soft-duplicate check against all other `cold_leads` rows (server endpoint `/cold-leads/api/check-duplicate`) and surfaces matches in a non-blocking toast — does not block save. Hot-lead checkbox tints the row green.
...
- `templates/cold_leads.html` — Xtime Follow Up Calls spreadsheet page. Inline-edit table with auto-save, phone auto-formatting on blur, soft-duplicate toast, and 100-row ghost-row padding (ghosts aren't persisted until typed in).
...
- `cold_leads` — one row per Xtime follow-up prospect. Columns: `market` (one of the 5 stores in `COLD_LEAD_MARKETS`), `name`, `phone` (stored in display format `(NNN-NNN-NNNN)` — formatted client-side on blur), `source` (`Xtime`/`Sales`/`Other`, nullable), `lead_date` (when the lead came in), `contact_date` (when we reached them), `notes`, `hot_lead` (boolean — green-tints the row in the UI). All fields are nullable except market — the page renders ghost rows that aren't persisted until the user types something in. The duplicate check (`find_cold_lead_duplicates`) matches name (case-insensitive) and phone (digits-only, requires ≥10 digits) across all markets.
