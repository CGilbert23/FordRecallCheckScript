...
- **Account Management** — customer accounts and the VIN lists saved against them ("Saved VIN Lists")
...
## Routes (high-level)
- `/` — home tile page (Account Management / Recall Checker / Used Car Tracker / Mobile Keys / Dealership Locator)
- `/accounts` and `/accounts/new`, `/accounts/<id>/edit`, `/accounts/<id>/delete` — labelled **Saved VIN Lists** in the UI (nav tab, page title, home tile link); the URLs and route names are unchanged.
...
- `/mobile-keys/import` (GET/POST) — **Import Invoice**. Reachable from the "Import Invoice" nav tab under Mobile Keys and the button beside "+ Add Key" on the Key Database list. Upload a CDK parts invoice PDF; POST parses it, matches each part group to a `mobile_keys` row, and renders the review page. No DB writes on this route. Uploads are read into memory only (never hit disk), and validated in `_read_invoice_upload`: `.pdf` name, non-empty, ≤ `MAX_INVOICE_BYTES` (10 MB), and a `%PDF` magic-byte check. The size cap lives there rather than on `MAX_CONTENT_LENGTH`, which would also constrain the Excel upload on `/submit`. Matching runs against active **and** inventory keys (`list_mobile_keys() + list_mobile_keys_in_inventory()`). The review page splits results into three buckets: **Will update** (key has no part data yet — pre-checked), **Already has part data** (unchecked; ticking one *replaces* what's saved), and **No matching key** (listed only, no checkbox). Each matched row shows editable part number/cost inputs plus before/after Total Parts and Total Cost from `_compute_key_totals()`.
...
- `db.py` — Supabase client + CRUD for schedules/accounts/leads/mobile_keys + constants (`MARKETS`, `ACCOUNT_REPS`, `SERVICE_TYPES`, `LEAD_TYPES`, `INTEREST_LEVELS`, `INTEREST_LEVEL_DEFAULT`, `LEAD_ATTEMPT_OUTCOMES`, `LEAD_CLOSE_REASONS`, `CADENCES`, `KEY_*`). `LOCATIONS` is an alias for `MARKETS`. `_digits_only` is shared by `find_duplicate_matches` — don't delete it as a lead helper. `bulk_update_mobile_keys(updates)` (used only by the invoice import) applies a list of partial `{'id': ..., **fields}` updates as a loop of single-row updates — Supabase has no per-row bulk update — and returns `(applied_count, errors)` so one bad row doesn't abort the batch. An update against a deleted id succeeds silently, so it counts `res.data` rather than attempts.
...
- `templates/_nav.html` — shared two-level nav partial; included by all pages. The Account Management group is now a single tab (Saved VIN Lists).
...
- `accounts` — master record for an existing customer (company, market, account_rep, fleet manager contact, service_type, VINs, notes). Supports an optional second fleet manager (`fleet_manager_2`, `fleet_manager_2_email`, `fleet_manager_2_phone`). The 2026-08-10 migration dropped the lead-source columns (`lead_source`, `lead_source_other`, `source_contact`) and the check-in columns (`last_checked_in_at`, `check_in_note`) — the Source and Recent Check In columns and the `/accounts/<id>/check-in*` routes were removed with them.
...
- `2026-08-10_remove_lead_tracking_notes_checkin.sql` is **destructive and order-sensitive**: it `drop table`s `cold_leads` and `notepad` (export first if the data matters) and drops five `accounts` columns. Run it *after* the matching code is deployed — the old code writes `accounts.lead_source` on every save, so running it against a still-running old container breaks account saves until the deploy lands.
...
- The Lead Tracking (cold leads) page and the shared `/notes` scratchpad were removed in the 2026-08-10 cleanup, along with `appt_importer.py` (the "Upload Appointments" fleet-prospecting importer), `templates/cold_leads.html`, `templates/notes.html`, the `/cold-leads*` and `/notes` routes, their `db.py` helpers, and the sample export under `uploadfiles/Appts/`. Unlike `account_leads`, the backing `cold_leads` and `notepad` tables were **dropped** — that data is gone unless it was exported first.
