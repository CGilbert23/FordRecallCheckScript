...
- `/mobile-keys/<id>/toggle-status` (POST) — flips the per-row `status_done` checkbox in the leftmost "Complete" column. Unchecked rows render with a light-grey row tint.
- `/mobile-keys/<id>/toggle-ordered` (POST) — flips the per-row `ordered` checkbox in the "Ordered" column (immediately right of Complete). Same checkbox style; does not tint the row.
